...
# #replace in estonian translation for LCD
# if [ "$LNG" = "et" ]; then
# #TODO
#  #map 'ä' to LCD font
...
#  #replace 'õ' with 'o'
#  sed -i 's/\xc3\xb5/o/g' $LNG'_filtered.po'
#  #replace 'Ü' with 'ü'
...
#replace in estonian translation for OLED
if [ "$LNG" = "et" ]; then
#TODO
 #map 'ä' to OLED font
 sed -i 's/\xc3\xa4/\x84/g' $LNG'_filtered.po'
 #map 'Ä' to OLED font
 sed -i 's/\xc3\xa4/\x8E/g' $LNG'_filtered.po'
 #map 'ö' to OLED font
 sed -i 's/\xc3\xb6/\x94/g' $LNG'_filtered.po'
 #map 'Ö' to OLED font
 sed -i 's/\xc3\xb6/\x99/g' $LNG'_filtered.po'
 #map 'ü' to OLED font
 sed -i 's/\xc3\xbc/\x81/g' $LNG'_filtered.po'
 #map 'Ü' to OLED font
 sed -i 's/\xc3\x9c/\x9a/g' $LNG'_filtered.po'
 #map 'õ' to OLED font
 sed -i 's/\xc3\xb5/\xad/g' $LNG'_filtered.po'
 #map 'Õ' to OLED font
 sed -i 's/\xc3\xb5/\xac/g' $LNG'_filtered.po'
 #replace 'ž' with 'z'
 sed -i 's/\xc5\xbe/z/g' $LNG'_filtered.po'
fi
...
#  /bin/echo -e "$s"
  s=$(/bin/echo -e "$s")
  s2=$(grep -F -A1 -B0 -a  "$s" "$LNG"_new.po | tail -n1 | sed 's/^msgstr //')
  if [ -z "$s2" ]; then
   echo '"\x00"'
  else
   echo "$s2"
